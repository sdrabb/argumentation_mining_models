...
function treelstm.read_trees(parent_path, label_path, sents_id_path)
  local parent_file = io.open(parent_path, 'r')
  local sents_id_file = io.open(sents_id_path, 'r')
  ...
  --   n_trees = n_trees +1
    ...
    count = count + 1
    trees[count] = treelstm.read_tree(parents, labels , tonumber(id))
  end
  parent_file:close()
  return trees
end

function treelstm.read_tree(parents, labels, dataset_idx)
  local size = #parents
  local trees = {}
  ...
  for i = 1, size do
    ...
    if tree ~= nil and tree.num_children == 0 then
      ...
      --added to check leaves consistency between leaves and toks
      root.num_leaves = leaf_idx

      leaf_idx = leaf_idx + 1

    end
  end
  root.dataset_idx = dataset_idx
  return root
end
...
function treelstm.read_sentiment_dataset(dir, vocab, fine_grained, dependency, unconsistences_file_path)
  local unconsistence_set_idx = {}
  local dataset = {}
  dataset.vocab = vocab
  ...
  if dependency then
    ...
    trees = treelstm.read_trees(dir .. 'parents.txt', dir .. 'labels.txt', dir .. 'sents_ids.txt')
  ...
  if not fine_grained then
    ...
    dataset.sents = {}
    for i = 1, #trees do
      -- print(sents[i]:size(1))
      -- print(trees[i].num_leaves)

      if trees[i].gold_label ~= 0 and sents[i]:size(1) == trees[i].num_leaves then
        table.insert(dataset.trees, trees[i])
        table.insert(dataset.sents, sents[i])
      else

        table.insert(unconsistence_set_idx, trees[i].dataset_idx)
      end
    end
  else
    dataset.trees = trees
  ...
  save_unconsistence_idx(unconsistence_set_idx,unconsistences_file_path)
