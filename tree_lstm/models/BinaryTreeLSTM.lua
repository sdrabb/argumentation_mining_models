...
-- this condider only the label on the root
...
function BinaryTreeLSTM:forward(tree, inputs)
  ...
  if tree.num_children == 0 then
    ...
    local lvecs= self:forward(tree.children[1], inputs)
    local rvecs = self:forward(tree.children[2], inputs)
  ...
  local loss = 0
  if self.output_module ~= nil and tree.is_root  then

    self:allocate_module(tree, 'output_module')
    tree.output = tree.output_module:forward(tree.state[2])
    if self.train then
      -- compute only the output respect to the root

	loss = self.criterion:forward(tree.output, tree.gold_label)
...
-- old forward with all nodes labeled
...
-- backward with only root labeled
...
function BinaryTreeLSTM:_backward(tree, inputs, grad, grad_inputs)
  ...
  if tree.output ~= nil and tree.gold_label ~= nil then
    -- compute only the output grad respect to the root
...
-- backward with all nodes labeled
